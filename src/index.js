...
*/
function render(contacts = window.contacts) {
...
function filterByCity(city) {
  ...
  const usersWithCity = window.contacts.filter(user => user.address.city === city);
  if (usersWithCity.length > 0) {
    document.querySelector("#contacts").innerHTML = usersWithCity.map(renderContact).join("");
...
*/
function loadCities(contacts = window.contacts) {
  const cities = [];
  document.querySelector("#filterOptions").innerHTML = '<option value="0">-- Select a city --</option>';
  ...
  if (contacts.length > 0) {
    ...
    cities.map(city => {
      ...
      addOption.setAttribute("value", `${city}`);
      addOption.innerHTML = `${city}`;

      document.querySelector("#filterOptions").appendChild(addOption);
    })
  } 
}
...
function deleteContact(id) {
  const index = window.contacts.findIndex(user => user.id == id);
  console.log(id)
  console.log(index)
  if (index >= 0) {
...
function deleteButtonHandler(event) {
  if (event.target && event.target.nodeName === "BUTTON") {
    const contactClicked = event.target.parentNode;
    const id = contactClicked.getAttribute("data-id")
    deleteContact(id);
    render();
...
function main() {
  ...
  const selectCity = document.querySelector("#filterOptions");
  selectCity.addEventListener("change", filterHandler);
